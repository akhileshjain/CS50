...
void grayscale(int height, int width, RGBTRIPLE image[height][width])
{
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
            ...
            // Take average of RGB values and assign the same to the current pixel.
            float average = round((blue + green + red) / 3.000);
...
void sepia(int height, int width, RGBTRIPLE image[height][width])
{
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
            ...
            // Use the Sepia formula to calculate sepia red, sepia green and sepia blue
            BYTE sepiaRed = round((0.393 * red) + (0.769 * green) + (0.189 * blue)) > 255 ? 255 : round((0.393 * red) + (0.769 * green) +
                            (0.189 * blue));
            BYTE sepiaGreen = round((0.349 * red) + (0.686 * green) + (0.168 * blue)) > 255 ? 255 : round((0.349 * red) + (0.686 * green) +
                              (0.168 * blue));
            BYTE sepiaBlue = round((0.272 * red) + (0.534 * green) + (0.131 * blue)) > 255 ? 255 : round((0.272 * red) + (0.534 * green) +
                             (0.131 * blue));
